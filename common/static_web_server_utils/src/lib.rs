...
mod tests {
    ...
    fn reads_project_toml() {
        let project_config = read_project_config(PathBuf::from("../../buildpacks/static-web-server/tests/fixtures/custom_doc_root").as_path())
            .unwrap()
            .expect("TOML value");
        let project_table = project_config.as_table()
            .expect("TOML table");
        assert_eq!(project_table.get("build"), None);
        assert_eq!(project_table.get("root"), Some(&toml::Value::String(String::from("foobar"))));
    ...
    #[test]
    fn no_project_toml() {
        let project_config = read_project_config(PathBuf::from("../../buildpacks/static-web-server/tests/fixtures/no_project_toml").as_path())
            .unwrap();
