...
const BUILDPACK_NAME: &str = "Heroku Website (create-react-app) Buildpack";

pub(crate) struct WebsiteCreateReactAppBuildpack;

impl Buildpack for WebsiteCreateReactAppBuildpack {
    type Platform = GenericPlatform;
    type Metadata = GenericMetadata;
    type Error = WebsiteCreateReactAppBuildpackError;
        ...
        let contents = match std::fs::read_to_string(context.app_dir.join("package.json")) {
            ...
            Err(e) => {
                return Err(libcnb::Error::BuildpackError(
                    WebsiteCreateReactAppBuildpackError::ReadPackageJson(e),
                ))
            }
        };
...
impl From<WebsiteCreateReactAppBuildpackError>
    for libcnb::Error<WebsiteCreateReactAppBuildpackError>
{
    fn from(value: WebsiteCreateReactAppBuildpackError) -> Self {
        libcnb::Error::BuildpackError(value)
    }
}

buildpack_main!(WebsiteCreateReactAppBuildpack);
