...
pub(crate) fn caddy_json_config(config: HerokuWebServerConfig) -> serde_json::Value {
    ...
    if let Some(headers) = config.headers {
        tracing::info!( { CONFIG_RESPONSE_HEADERS_ENABLED } = true, "caddy config");
    ...
    {
        tracing::info!({ CONFIG_CADDY_SERVER_OPTS_BASIC_AUTH } = true, "caddy config");
    ...
    {
        tracing::info!({ CONFIG_CADDY_SERVER_OPTS_TEMPLATES } = true, "caddy config");
        static_file_handlers.push(json!(
        {
            "handler": "templates",
        }));
    ...
    if caddy_access_logs_config.is_some_and(|vv| vv.enabled.is_some_and(|vvv| vvv)) {
        tracing::info!({ CONFIG_CADDY_SERVER_OPTS_ACCESS_LOGS } = true, "caddy config");
...
fn generate_error_404_route(
    ...
    let not_found_response_handlers = error_config
        .map(|error_config| {
            ...
            tracing::info!({ CONFIG_ERROR_404_FILE_PATH } = error_config.file_path.to_string_lossy().to_string(), "error 404 file path");
            tracing::info!({ CONFIG_ERROR_404_STATUS_CODE } = status_code, "error 404 status code");
